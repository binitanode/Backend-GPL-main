...
//         const proxyServer = 'socks5://2.56.119.93:5074';

//         const proxyHost = '2.56.119.93';
//         const proxyPort = '5074';
//         const proxyUsername = 'hgfcnpfq';
//         const proxyPassword = '4q1umwtss5q4';
...
async function scrapping_createDetail(req, res) {
  try {
       console.log("scrapping_createDetail api call");
    var url_arr = [];
    var ids_arr = [];
    var getalllink = [await VendorModel.findOne({ $or: [{ process_status: { $exists: false } },{ process_status: false }]})];
    console.log("getalllink",getalllink);
    if (getalllink) {
      for (let g = 0; g < Math.min(getalllink.length, 1); g++) {
          url_arr.push({ url: getalllink[g].profileLink, idds: getalllink[g]._id });
      }
    }
    console.log("url_arr", url_arr);
    ...
    if (url_arr) {
      try {
        // const proxylist = await listProxies();
        ...
        for (const { url, idds } of urls) {
          ...
          const data = {
            ...
            venor_profile_link: url,
            venor_profile_title: title,
            avg_completion_rate: values && values.length >= 1 ? values[0] : "",
            task_initial_domain_price: values && values.length >= 2 ? values[1] : "",
            avg_lifetime_link: values && values.length >= 3 ? values[2] : "",
            no_of_complete_task: '',
            no_of_task_progress: '',
            no_of_rejected_task: '',
            no_of_approved_sites: '',
